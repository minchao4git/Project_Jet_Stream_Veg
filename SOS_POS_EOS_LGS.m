function []=SOS_POS_EOS_LGS()
	...
	sos_map=nan(s1,s2,nds);
    pos_map=nan(s1,s2,nds);
    eos_map=nan(s1,s2,nds);
    lgs_map=nan(s1,s2,nds);
    ...
    for dsid=1:nds
        ...
        for i=1:s1
            for j=1:s2
                ...
                % FAO approach lead to no SOS for southern Europe
                ...
                frs_days=15;
                for m=1:35
                    % left shoulder: greening phase, increasing trend
    ...
    for dsid=1:length(ds_rng)
        ...
        figure('color','w','Position',[516         327        1124         445]);
        gap_h=0.005; gap_w=0.002;
        gap=[gap_h gap_w]; marg_h=[0.08 0.05]; marg_w=[0.1 0.05];
        ha = tight_subplot(1,4,gap,marg_h,marg_w);
