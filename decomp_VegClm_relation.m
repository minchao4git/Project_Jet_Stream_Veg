function decomp_VegClm_relation(dsid)

    % Decompose the climate dirvers contribution to the interannual
    % variability of vegetation index
    % only for three input variables
    
    global DATA_VI_05rs_dtds_out_gs1 DATA_CLM_05rs_dtds_out_gs1;
    global lgs_map;
    global mon_vegclm_par_r mon_vegclm_par_p;
%     global mon_vegclm_r mon_vegclm_p;
    
    [s1 s2 s3 s4 s5]=size(DATA_CLM_05rs_dtds_out_gs1);
   
    dtmp_v=DATA_VI_05rs_dtds_out_gs1;  % Growing season months
    dtmp_c=DATA_CLM_05rs_dtds_out_gs1(:,:,:,:,1:3);
