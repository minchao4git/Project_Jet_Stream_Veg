function [] = preproc_calcLocalClim(temp_int,pwi,use_gs,dsid)
    % Preprocessing local climate data for detrend and deseasonalization
    % Growing season data: detrend
    % Monthly data: detrend and deseasonalize

    % Parameters:
    % 1. Temporal interval
    % temp_int={'biweekly_dsclim'}; % growssn, monthly, monthly_dsclim, biweekly_dsclim, biweekly_dsmvm
    % 2. Prewhitening
    % pwi=1: apply prewhitening, pwi=0: don't apply prewhitening
    ...
    % zonal-mean zonal wind from ERA5
    data_ts(:,:,1:s3*s4,4)=reshape(DATA_ERA5_05rs_out(:,:,:,:,7),[s1 s2 s3*s4 1]);
...
%                       m_s=(y-1)*12+gssn_s;
    ...
    if strcmp(temp_int,'monthly_dsclim')
        ...
        for v = 1:nv
            % 1. using CDT toolkits
            % For 3D array, deseason along the 3rd dimension by default
            % (does not support array with dim >=3)
            % problematic when some monthes are missing
            % [dtmp ssn1]=(deseason1(squeeze(data_in(:,:,:,dsid)), (TT.Time)'));
            % DATA_VI_05rs_dtds_out(:,:,:,dsid)=dtmp;
        ...
        % The detrend and deseasonalize process here regards 12 month mean as the
        % growing season mean, may overestiamte the mean for the area with
        % season 2 growing season, while our analysis focus on the season
        % 1 data point. 
        % A better way is to consider the season 1 only for this dataset.
        % but meanwhile, since we only focus on the anomalies based on a
        % fixed climatology, therefore, it won't affect our conclusion
        % based on the anomalies calculation.
        if use_gs
