function r_by_lc_clmzone()
    ...
    v_in=4; % 4 is EVI2
    nv=3;
    data_calc(:,:,1,1)=squeeze(v_clminx_r(:,:,13,1,v_in)); % 1st half
    data_calc(:,:,2,1)=squeeze(v_clminx_r(:,:,14,1,v_in)); % 2nd half
    data_calc(:,:,3,1)=squeeze(v_clminx_r(:,:,15,1,v_in)); % 15 for entire GS
    
    data_calc(:,:,1,2)=squeeze(v_clminx_p(:,:,13,1,v_in)); % 1st half
    data_calc(:,:,2,2)=squeeze(v_clminx_p(:,:,14,1,v_in)); % 2nd half
    data_calc(:,:,3,2)=squeeze(v_clminx_p(:,:,15,1,v_in)); % 15 for entire GS
    ...
    for clm=1:7
        for lc=1:8
            ...
            for v=1:3  % variable presented in the calculation
                ...
                rtmp=squeeze(dtmp(:,:,v,1)); % correlation coefficient
                ptmp=squeeze(dtmp(:,:,v,2)); % significance
                ...
                btmp=squeeze(nanmean(dtmp(:,:,v,1),4)); % ds=1, r values
    ...
    for gi=1:3
        ...
        for clm_grp=clm_grp_grp{gi}
            ...
            for bi=1:3
                ...
                % adjust the * position for positive and negative values, mask out the non-significant value
                % fixed heigh
            ...
            sgtitle(sprintf('GS-based EVI2~%s(r) by climate zone and land class\n',upper(clmidx_name)),'Fontsize',12, 'FontWeight','bold');
